...
CONFIRM_ON_QUIT = SetTup(
    "prefs/confirm_on_quit", True, "Confirm when quitting with unprocessed items"
)
ALLOW_NO_SETTXT = SetTup(
    "prefs/allow_no_settxt", True, "Allow folders without settings.txt"
)
WARN_ITERS = SetTup(
    "prefs/warn_iters",
    True,
    'Ask whether to rename files when folder with "Iter_" is added',
)
SKIP_PROCESSED = SetTup(
    "prefs/skip_processed", False, "Skip folders that have already been processed"
)
SAVE_MIPS = SetTup(
...
ALLOW_BUGREPORT = SetTup(
    "pref/allow_bugreport", True, "Allow anonymous bug reports with errors"
)
