...
if not cudaLib:
    ...
    try:
        ...
        RL_interface.argtypes = [np.ctypeslib.ndpointer(ctypes.c_ushort, flags="C_CONTIGUOUS"),  # raw im
                        ctypes.c_int,                                                   # nx
                        ctypes.c_int,                                                   # ny
                        ctypes.c_int,                                                   # nz
                        np.ctypeslib.ndpointer(ctypes.c_float, flags="C_CONTIGUOUS"),
...
def RL_init(rawdata_shape, otfpath, drdata=0.104, dzdata=0.5, drpsf=0.104,
            dzpsf=0.1, deskew=31.5, rotate=0, width=0, deviceID=0, **kwargs):
    requireCUDAlib()
    nz, ny, nx = rawdata_shape
    RL_interface_init(nx, ny, nz, drdata, dzdata, drpsf, dzpsf, deskew,
                      rotate, width, otfpath.encode(), deviceID)
...
class RLContext(object):
    ...
    def __enter__(self):
        ...
        RL_interface_init(self.nx, self.ny, self.nz, self.dr, self.dz,
                          ...
                          self.rotate, self.width, self.otfpath.encode(), self.deviceID)
...
def RL_decon(im, background=80, nIters=10, shift=0, savedeskew=False,
    ...
    RL_interface(im, nx, ny, nz, decon_result, deskew_result,
                 background, rescale, savedeskew, nIters, shift, 0, 0, 0.0, False, 0)
